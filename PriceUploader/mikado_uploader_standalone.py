#!/usr/bin/env python3
"""
Автономная программа для загрузки прайса Mikado
...
class MikadoUploaderStandalone:
    def __init__(self, root):
        self.root = root
        self.root.title("Загрузчик прайса Mikado")
        ...
        self.server_url = tk.StringVar(value="http://localhost")
    ...
    def setup_ui(self):
        ...
        title_label = ttk.Label(main_frame, text="Загрузчик прайса Mikado", font=("Arial", 16, "bold"))
        ...
        self.status_label = ttk.Label(status_frame, textvariable=self.status_var, wraplength=600)
    ...
    def upload_file(self):
        try:
            ...
            api_url = f"{server_url}/api/upload-mikado/"
    ...
    def monitor_progress(self, server_url):
        try:
            progress_url = f"{server_url}/api/mikado-upload-progress/"
    ...
    def update_brands(self):
        try:
            ...
            api_url = f"{server_url}/api/update-mikado-brands/"
    ...
    def monitor_brand_update_progress(self, server_url):
        try:
            progress_url = f"{server_url}/api/update-mikado-brands-progress/"
            while self.is_uploading:
                response = requests.get(progress_url, timeout=10)
                if response.status_code == 200:
                    data = response.json()
...
def main():
    root = tk.Tk()
    app = MikadoUploaderStandalone(root)
    root.mainloop()
