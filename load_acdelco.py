...
# Фильтруем товары ACDelco
acdelco_df = df[df['BrandName'] == 'ACDelco'].head(5)

print(f'Найдено товаров ACDelco в прайсе: {len(acdelco_df)}')

# Добавляем товары в базу
added = 0
for _, row in acdelco_df.iterrows():
    try:
...
print(f'\nВсего добавлено товаров ACDelco: {added}')
