...
# Фильтруем товары ZF Parts
zf_parts_df = df[df['BrandName'] == 'ZF Parts'].head(5)

print(f'Найдено товаров ZF Parts в прайсе: {len(zf_parts_df)}')

# Добавляем товары в базу
added = 0
for _, row in zf_parts_df.iterrows():
    try:
...
print(f'\nВсего добавлено товаров ZF Parts: {added}')
