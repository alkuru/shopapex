...
class Command(BaseCommand):
    help = 'Загружает 5 товаров бренда CEI из прайса AutoKontinent'

    def handle(self, *args, **options):
        """Загружает 5 товаров бренда CEI из прайса AutoKontinent"""
        ...
        try:
            ...
            # Проверяем результат
            total_cei = AutoKontinentProduct.objects.filter(brand__icontains='CEI').count()
            self.stdout.write(f"📋 Всего товаров CEI в базе: {total_cei}")
