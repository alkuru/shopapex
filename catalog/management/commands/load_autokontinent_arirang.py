...
class Command(BaseCommand):
    help = 'Загружает 5 товаров бренда Arirang из прайса AutoKontinent'

    def handle(self, *args, **options):
        """Загружает 5 товаров бренда Arirang из прайса AutoKontinent"""
        ...
        try:
            ...
            # Фильтруем по бренду Arirang
            brand_filter = 'Arirang'
            ...
            # Проверяем результат
            total_arirang = AutoKontinentProduct.objects.filter(brand__icontains='Arirang').count()
            self.stdout.write(f"📋 Всего товаров Arirang в базе: {total_arirang}")
