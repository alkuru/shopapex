...
class Command(BaseCommand):
    help = 'Загружает 5 товаров бренда Continental из прайса AutoKontinent'

    def handle(self, *args, **options):
        """Загружает 5 товаров бренда Continental из прайса AutoKontinent"""
        ...
        try:
            ...
            # Фильтруем по бренду Continental
            brand_filter = 'Continental'
            ...
            # Проверяем результат
            total_continental = AutoKontinentProduct.objects.filter(brand__icontains='Continental').count()
            self.stdout.write(f"📋 Всего товаров Continental в базе: {total_continental}")
