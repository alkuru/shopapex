...
class Command(BaseCommand):
    help = 'Загружает 5 товаров бренда Bendix из прайса AutoKontinent'

    def handle(self, *args, **options):
        """Загружает 5 товаров бренда Bendix из прайса AutoKontinent"""
        ...
        try:
            ...
            # Проверяем результат
            total_bendix = AutoKontinentProduct.objects.filter(brand__icontains='Bendix').count()
            self.stdout.write(f"📋 Всего товаров Bendix в базе: {total_bendix}")
