...
class Command(BaseCommand):
    help = 'Загружает 5 товаров бренда AllBalls из прайса AutoKontinent'

    def handle(self, *args, **options):
        """Загружает 5 товаров бренда AllBalls из прайса AutoKontinent"""
        ...
        try:
            ...
            # Фильтруем по бренду AllBalls
            brand_filter = 'AllBalls'
            ...
            # Проверяем результат
            total_allballs = AutoKontinentProduct.objects.filter(brand__icontains='AllBalls').count()
            self.stdout.write(f"📋 Всего товаров AllBalls в базе: {total_allballs}")
