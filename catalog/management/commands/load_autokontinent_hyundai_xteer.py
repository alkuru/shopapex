...
class Command(BaseCommand):
    help = 'Загружает 5 товаров бренда Hyundai Xteer из прайса AutoKontinent'

    def handle(self, *args, **options):
        """Загружает 5 товаров бренда Hyundai Xteer из прайса AutoKontinent"""
        ...
        try:
            ...
            # Фильтруем по бренду Hyundai Xteer
            brand_filter = 'Hyundai Xteer'
            ...
            # Проверяем результат
            total_hyundai_xteer = AutoKontinentProduct.objects.filter(brand__icontains='Hyundai Xteer').count()
            self.stdout.write(f"📋 Всего товаров Hyundai Xteer в базе: {total_hyundai_xteer}")
